...
def create_prompt(categories, cfg, tokenizer, model, img_paths):
    ...
    # prompt_dict = {
    #     'color': [f'What is the color of the {label_list[0][0]} skin lesion?',
    #               f'What is the color of the {label_list[0][1]} skin lesion?'],
    #     'shape': [f'What is the shape of the {label_list[0][0]} skin lesion?',
    #               f'What is the shape of the {label_list[0][1]} skin lesion?'],
    #     'location': [f'Where is this {label_list[0][0]} skin lesion located on?',
    #                  f'Where is this {label_list[0][1]} skin lesion located on?']
    # }
    prompt_dict = {
        'color': [f'What is the color of the skin lesion?',
                  f'What is the color of the melanoma?'],
        'shape': [f'What is the shape of the skin lesion?',
                  f'What is the shape of the melanoma?'],
        # 'location': [f'What surface is this skin lesion located on?',
        #              f'What surface is this melanoma located on?']
    }

    # with open('/root/projects/vl/VL-model-medical-image-understanding/prompt_dfuc_logs.txt', 'a+') as f:
    #     f.writelines(f'color: What is the color of this wound? \n')
    #     f.writelines(f'shape: What is the shape of this wound? \n')
    #     f.writelines(f'location: What surface is this wound located on?')
    ...
    for img_path in img_paths:
        ...
        for i in range(len(label_list[0])):
            ...
            caption += [f'{color} color, {shape} shape, {cls_list[i]} in the skin']
        captions.append('. '.join(caption))
    # print('**************'*10)
    # print(f'Paths: {img_paths}')
    # print(f'Captions: {captions}')
    with open('/root/projects/vl/VL-model-medical-image-understanding/prompt_isbi_logs.txt', 'a+') as f:
        f.writelines('**************'*7+ '\n')
            ...
            if cfg.TEST.USE_MULTISCALE:
                ...
                for query_i in range(query_time):
                    ...
                    if task == "detection":
                        ...
                        #weird_list.append(batch[3])

                        # if i == 189:
                        #     import pdb; pdb.set_trace()
                        #     s = set()
                        #     for i, w in enumerate(weird_list):
                        #         if len(w) < 2:
                        #             print('hhhhhhhhhhhwcnmd')
                        #             print(w, i)
                        #             break
                        #         if w[0] in s:
                        #             print(w[0])
